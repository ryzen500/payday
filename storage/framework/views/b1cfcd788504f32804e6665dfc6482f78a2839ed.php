    ...
    var table_table = $('#work_experience-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "<?php echo e(route('work_experience.show',$employee->id)); ?>",
        },
    ...
    $('#create_work_experience_record').click(function () {
        ...
        $('.modal-title').text('<?php echo e(__('Add Work Experience')); ?>');
        $('#work_experience_action_button').val('<?php echo e(trans('file.Add')); ?>');
        $('#work_experience_action').val('<?php echo e(trans('file.Add')); ?>');
        $('#WorkExperienceformModal').modal('show');
    });

    $('#work_experience_sample_form').on('submit', function (event) {
        event.preventDefault();
        if ($('#work_experience_action').val() == '<?php echo e(trans('file.Add')); ?>') {

            $.ajax({
                url: "<?php echo e(route('work_experience.store',$employee->id)); ?>",
                method: "POST",
        ...
        if ($('#work_experience_action').val() == '<?php echo e(trans('file.Edit')); ?>') {
            $.ajax({
                url: "<?php echo e(route('work_experience.update')); ?>",
                method: "POST",
                data: new FormData(this),
                contentType: false,
                cache: false,
    ...
    $(document).on('click', '.work_experience_edit', function () {
        ...
        var target = "<?php echo e(route('work_experience.index')); ?>/" + id + '/edit';
    ...
    $('#work_experience_hidden_id').val(html.data.id);
                ...
                $('#work_experience_action_button').val('<?php echo e(trans('file.Edit')); ?>');
                $('#work_experience_action').val('<?php echo e(trans('file.Edit')); ?>');
                $('#WorkExperienceformModal').modal('show');
            }
        })
    });
    ...
    experience_delete_id = $(this).attr('id');
        ...
        $('.experience-ok').text('<?php echo e(trans('file.OK')); ?>');
    });
    ...
    $('.experience-ok').click(function () {
        let target = "<?php echo e(route('work_experience.index')); ?>/" + experience_delete_id + '/delete';
        $.ajax({
            url: target,
            beforeSend: function () {
                $('.experience-ok').text('<?php echo e(trans('file.Deleting...')); ?>');
            },
            success: function (data) {
                setTimeout(function () {
                    $('.confirmModal').modal('hide');
...
<?php /**PATH /opt/bitnami/apache2/htdocs/demo/payday/resources/views/employee/work_experience/index_js.blade.php ENDPATH**/ ?>