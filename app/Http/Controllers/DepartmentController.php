...
class DepartmentController extends Controller {
	...
	public function index()
	{
		...
		$parent =  department::select('id', 'department_name')
				->get();

		$logged_user = auth()->user();
		$employees = Employee::select('id', 'first_name', 'last_name', 'company_id')->where('email', $logged_user->email)->get();
		$company_id = 0;
		foreach($employees as $employee) {
			$company_id = $employee->company_id;
		}
		$department_head = Employee::select('id', 'first_name', 'last_name')->where('company_id', $company_id)->get();
		if (request()->ajax())
		{
		...
		return view('organization.department.index', compact('companies', 'parent', 'employees', 'department_head'));
	}
