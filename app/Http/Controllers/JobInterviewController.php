...
class JobInterviewController extends Controller {
			...
			if (request()->ajax())
			{
				...
				return datatables()->of($job_interviews)
					...
					->addColumn('action', function ($data)
					{
						...
						$button .= '<button type="button" name="accepted" id="' . $data->id . '" class="accepted btn btn-primary btn-sm">Accept</button>';
						$button .= '&nbsp;&nbsp;';
	...
	public function show($id)
	{
		if (request()->ajax())
		{
			...
			{
				$cand = JobCandidate::with('AppliedJob')->findOrFail($candidate->id);

				$data['job_title']= $cand->AppliedJob->job_title;
				$data['short_description']= $cand->AppliedJob->short_description;
				$data['job_employer'] = $candidate->full_name;
				$data['company_name'] = $cand->AppliedJob->PostJobEmployer->company_name ?? '';
	...
	{	
		...
		$data = JobInterview::with(
				'employees:id,first_name,last_name', 'candidates:id,full_name,email,phone,address', 'User:id,username')->findOrFail($id);
		...
		JobInterview::whereId($id)->delete();
		JobCandidate::whereId($candidate_id)->delete();

		return response()->json(['success' => __('Data is successfully accept')]);
	}
}
