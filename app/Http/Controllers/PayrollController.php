		...
		if ($logged_user->can('view-paylist'))
		{
			...
			$employees = Employee::with(
				...
				return datatables()->of($employees)
					...
					{
						// foreach($row->employeeBankAccount as $bank_account){
					...
					->addColumn('account_number', function ($row)
					{
						...
						// return $row->employeeBankAccount;
					...
					->addColumn('basic_salary', function ($row) use ($first_date)
					{
						...
						return $basicsalary;
                                    ...
                                    ->addColumn('allowance_total', function ($row) use ($first_date)
					{
						$allowance_amount = 0;
						$first_date = '';
                        foreach ($row->allowances as $allowance) {
                            $first_date = $allowance->first_date;
                        }
						
						$type          = "getAmount";
						$allowances    = $this->allowances($row, $first_date , $type);
						$allowance_amount = $allowances;
						return 'Rp. '.$allowance_amount;
					})
					->addColumn('commission_total', function ($row) use ($first_date)
					{
						$commission_amount = 0;
                        foreach ($row->commissions as $commission) {
                            if($commission->first_date <= $first_date)
                            {
                                $commission_amount = $commission->commission_amount; //basic salary
                            }
                        }
						return 'Rp. '.$commission_amount;
					})
					->addColumn('loans_total', function ($row) use ($first_date)
					{
						$loans_amount = 0;
                        foreach ($row->loans as $loans) {
                            if($loans->first_date <= $first_date)
                            {
                                $loans_amount = $loans->loan_amount; //basic salary
                            }
                        }
						return 'Rp. '.$loans_amount;
					})
					->addColumn('deductions_total', function ($row)
					{
                        ...
                        foreach ($row->deductions as $deductions) {
                            $first_date = $deductions->first_date;
                        }
						$deductions    = $this->deductions($row, '2022-03-01', $type);
						$deductions_amount = $deductions;
						return 'Rp. '.$deductions_amount;
					})
					->addColumn('other_total', function ($row) use ($first_date)
					{
						$other_amount = 0;
                        foreach ($row->otherPayments as $otherPayments) {
                            if($otherPayments->first_date <= $first_date)
                            {
                                $other_amount = $otherPayments->loan_amount; //basic salary
                            }
                        }
			...
			return view('salary.pay_list.index', compact('companies','employees'));			
		}
