...
class PolicyController extends Controller {
	...
	public function store(Request $request)
	{
		...
		if ($logged_user->can('store-policy'))
		{
			...
			if($policy_file) {
				$policy_file_path = $request->policy_file;

				if($policy_file_path) {
					$policy_file_path = public_path('uploads/company_policy/' . $policy_file_path);
				...
				$ext = pathinfo($policy_file->getClientOriginalName(), PATHINFO_EXTENSION);
				$logoName = 'policy_'. date('Ymdhis') .'.' . $ext;
				$policy_file->move('public/uploads/company_policy/', $logoName);
				$data['policy_file'] =  $logoName;
			}
	...
	public function update(Request $request)
	{
		...
		if ($logged_user->can('edit-policy'))
		{
			...
			// $policyy = DB::table('policies')
			// 		->where('id', $id)
			// 		->get();

			if($policy_file) {
				// $policy_file_path = '';
				// foreach($policyy as $policys) {
				// 	$policy_file_path = $policys->policy_file;
				// }
