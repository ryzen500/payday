   ...
   var table_table = $('#commission-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('salary_commission.show',$employee->id) }}",
        },
    ...
    $('#create_commission_record').click(function () {
        ...
        $('.modal-title').text('{{__('Add Commission')}}');
        $('#commission_action_button').val('{{trans('file.Add')}}');
        $('#commission_action').val('{{trans('file.Add')}}');
    ...
    $(document).on('click', '.commission_edit', function () {
        ...
        var target = "{{ route('salary_commission.index') }}/" + id + '/edit';
        ...
        $.ajax({
            ...
            success: function (html) {
                ...
                $('#commission_action_button').val('{{trans('file.Edit')}}');
                $('#commission_action').val('{{trans('file.Edit')}}');
                $('#CommissionformModal').modal('show');
            }
        })
    });
    ...
    commission_delete_id = $(this).attr('id');
        ...
        $('.commission-ok').text('{{trans('file.OK')}}');
    });
    ...
    $('.commission-ok').click(function () {
        let target = "{{ route('salary_commission.index') }}/" + commission_delete_id + '/delete';
        $.ajax({
            url: target,
            beforeSend: function () {
                $('.commission-ok').text('{{trans('file.Deleting...')}}');
            },
            success: function (data) {
                setTimeout(function () {
                    $('.confirmModal').modal('hide');
