...
@section('content')
    ...
    <section>
        ...
        <div class="container-fluid mb-3">
            @can('store-department')
                <button type="button" class="btn btn-info" name="create_record" id="create_record"><i
                            class="fa fa-plus"></i> {{__('Add Department')}}</button>
            @endcan
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 id="exampleModalLabel" class="modal-title">{{__('Add Department')}}</h5>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="parent_id" id="parent_id"
                                        class="form-control"
                                        title="{{ __('Selecting Parent Departement') }}...">
                                        @foreach ($parent as $parents)
                                            <option value="{{$parents->id}}"> {{ $parents->department_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="employee_id" id="employee_id" class="selectpicker form-control"
                                        ...
                                        @foreach($department_head as $department_heads)
                                            <option value="{{$department_heads->id}}">{{$department_heads->first_name}} {{$department_heads->last_name}}</option>
                                        @endforeach 
                                    </select>
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        (function($) {  
            ...
            $(document).on('click', '.edit', function () {
                ...
                var target = "{{ route('departments.index') }}/" + id + '/edit';
            ...
            $(document).on('click', '#bulk_delete', function () {
                ...
                if (id.length > 0) {
                    if (confirm('{{__('Delete Selection',['key'=>trans('file.Department')])}}')) {
                        $.ajax({
                            url: '{{route('mass_delete_departments')}}',
                            method: 'POST',
                            data: {
            ...
            $('#ok_button').on('click', function () {
                let target = "{{ route('departments.index') }}/" + delete_id + '/delete';
                $.ajax({
                    url: target,
                    beforeSend: function () {
